...
public class MedicamentoDAO {
    ...
    public boolean adicionarComentario(int medicamentoId, Comentario comentario) {
        ...
        for (Medicamento m : listaDeMedicamentos) {
            if (m.getId() == medicamentoId) {
                List<Comentario> comentarios = m.getComentarios();
                int proximoId = 1;
    ...
    public boolean removerComentario(int medicamentoId, int comentarioId) {
        List<Medicamento> listaDeMedicamentos = lerDoArquivo();
        for (Medicamento m : listaDeMedicamentos) {
            if (m.getId() == medicamentoId) {
                List<Comentario> comentarios = m.getComentarios();
                Comentario comentarioAlvo = null;
                for (Comentario c : comentarios) {
                ...
                if (comentarioAlvo != null) {
                    comentarios.remove(comentarioAlvo);
                    salvarNoArquivo(listaDeMedicamentos);
                    return true;
                }
            }
